...
import { buildManagerContract, formatManagerContract } from '../server/managerContract.js';
...
function managerContract(args) {
  const apiBaseUrl = readFlag(args, '--api-base-url', process.env.AT_TEAM_API_BASE_URL || 'http://127.0.0.1:5174');
  const contract = buildManagerContract({ apiBaseUrl });
  if (hasFlag(args, '--json')) return contract;
  return formatManagerContract(contract);
}
...
async function main() {
  ...
  if (command === 'contract' || command === 'manager-contract') {
    const result = managerContract(args);
    if (typeof result === 'string') console.log(result.trimEnd());
    else json(result);
    return;
  }
