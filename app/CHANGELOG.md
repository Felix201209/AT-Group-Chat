...
- Added `at-group-chat init --all` to generate a complete repo integration bundle: Team as Code, GitHub workflow, external-manager prompt, env starter, MCP config, and OpenAPI JSON.
- Added a shared machine-readable manager contract for external AI agents and IDE tools through `at-group-chat contract --json`, `GET /api/contract`, SDK `at.contract()`, and MCP `team_get_manager_contract`.
